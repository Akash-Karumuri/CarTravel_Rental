...
const Cars = () => {
  ...
  if (loading) return <CarLoader message="Fetching cars..." />;
  ...
  return (
    <section className="container cars p-5 mt-5">
      ...
      <div className="row mt-5">
        {car?.map((item) => (
          <div data-aos="zoom-in" key={item?._id} className="col-md-4 mb-3">
            <figure className="shadow">
              <figcaption>
                ...
                <p className="px-3 py-2 m-0">
                  <strong>Seating Capacity:</strong> {item?.seatingCapacity}
                </p>

                <div className="d-flex justify-content-center my-3">
                  <NavLink to={`/CarDetails/${item?._id}`}>
                    <button className="btn btn-primary w-100">Know More</button>
                  </NavLink>
                </div>
              </figcaption>
            </figure>
